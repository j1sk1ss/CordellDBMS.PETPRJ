...
int DRM_lock_directory(directory_t* directory, uint8_t owner) {
    ...
    while (DRM_lock_test(directory, owner) == LOCKED)
        if (--delay <= 0) {
            print_error("Can't lock directory [%.*s], owner: [%i], new owner [%i]", DIRECTORY_NAME_SIZE, directory->header->name, directory->lock_owner, owner);
            return -1;
        }
...
int DRM_release_directory(directory_t* directory, uint8_t owner) {
    ...
    print_debug("Try to unlock directory [%.*s]", DIRECTORY_NAME_SIZE, directory->header->name);
