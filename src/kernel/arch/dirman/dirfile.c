        ...
        #endif
        {
            ...
            if (ci < 0) { print_error("Can`t create file: [%s]", save_path); }
                ...
                if (NIFAT32_write_buffer2content(
                    ci, offset, (const_buffer_t)encoded_header, sizeof(directory_header_t) * sizeof(unsigned short)
                ) != sizeof(directory_header_t) * sizeof(unsigned short)) status = -1;
                ...
                for (int i = 0; i < directory->header->page_count; i++) {
                    ...
                    if (NIFAT32_write_buffer2content(
                        ci, offset, (const_buffer_t)encoded_page_name, PAGE_NAME_SIZE * sizeof(unsigned short)
                    ) != PAGE_NAME_SIZE * sizeof(unsigned short)) {
                        status = -1;
                    }
...
unsigned int DRM_get_checksum(directory_t* directory) {
    ...
    if (directory->header) {
        _checksum = crc32(0, (const unsigned char*)directory->header, sizeof(directory_header_t));
    }

    directory->header->checksum = prev_checksum;
    _checksum = crc32(_checksum, (const unsigned char*)directory->page_names, sizeof(directory->page_names));
    return _checksum;
}
